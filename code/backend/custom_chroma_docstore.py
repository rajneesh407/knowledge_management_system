...
class ChromaStore(BaseStore[str, bytes]):
    """
    This is custom ChromaStore which will be used for docstore/byterstore to keep multiple summarized documents,
    related questions etc.
    The Langchain library does not provide this out of the box and thus a custom implementaion is needed.

    source: https://github.com/langchain-ai/langchain/issues/13909

    This code has been taken from this github repo:
    https://github.com/raelix/telegram-ai-bot/blob/main/vectorstore/chroma_docstore.py
    ...
    def _create_table(self):
        ...
        create_table_query = (
            Query.create_table(self.table)
            .columns(id_column, data_column)
            .if_not_exists()
        )
    ...
    def mget(self, keys: Sequence[str]) -> List[Optional[bytes]]:
        select_query = (
            ...
            .where(self.id_column.isin(keys))
        )
        with self.get_connection() as connection:
            cursor = connection.cursor()
    ...
    def mset(self, key_value_pairs: Sequence[Tuple[int, bytes]]) -> None:
        ...
        for key, value in key_value_pairs:
            insert_query = (
                ...
                .columns(self.id_column, self.data_column)
                .insert(key, json.dumps(value.decode("utf-8")))
            )
